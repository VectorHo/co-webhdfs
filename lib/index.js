...
var WebHdfsClient = module.exports = function(options) {
  ...
  this.options = _.defaults(options || {}, {
    ...
    namenode_port: 50070,
    datanode_port: 50070, // 安全机制时要覆盖1006
...
_.extend(WebHdfsClient.prototype, {
   ...
   */
  setActiveNamenodes: function(hosts) {
    if (!_.isArray(hosts)) hosts = [hosts];
    this.current_host = hosts[0];
    var protocol = this.options.protocol;
    var port = this.options.namenode_port;
    var path_prefix = this.options.path_prefix;
    this.base_urls = _.map(hosts, function(h) {
      return protocol + '://' + h + ':' + port + path_prefix;
    });
  },
  ...
  _replaceHost: function(url) {
    // var d_port = ':' + (this.options.datanode_port || 50075); // 50075 || 1006
    var url_obj = url_tool.parse(url);
    var hostname = url_obj.hostname;
    url_obj.hostname = this.options.datanode_hosts[hostname]; // 覆盖hostname
    url_obj.host = url_obj.hostname + ':' + url_obj.port;
    var url_str = url_tool.format(url_obj);
    console.log("%s --> %s", url, url_str);
    return url_str;
  },
    ...
    for (var i = 0; i < args.length; i++) {
      ...
      if (res.body && res.body.RemoteException) {
        ...
        try {
          ...
          err = new Error('WebHdfs: Invalid status code(' + res.statusCode +').');
        }
